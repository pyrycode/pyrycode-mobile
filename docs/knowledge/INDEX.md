...
- [ConversationRow](features/conversation-row.md) — stateless M3 `ListItem`-based row consuming `Conversation` + `Message?`; auto-name fallback branched on `isPromoted`, whitespace-normalized 2-line preview, kotlinx-datetime relative-time bucketing (`just now` / `Nm ago` / `Nh ago` / `Yesterday` / `Nd ago` / abbreviated date). Headline is a `Row` with two natural-width decorations: workspace label (#19, trailing) and sleeping-session dot (#20, leading — 8.dp `Box` filled with `colorScheme.outline`, gated on `Conversation.isSleeping`). Shared primitive for the #46 channel list and the discussions drilldown.
...
- [ChannelListScreen](features/channel-list-screen.md) — first stateless `(state, onEvent)` screen; M3 `Scaffold` with a `TopAppBar` (app-name title + trailing settings-gear `IconButton`, #21) wraps the four `ChannelListUiState` variants (centred-`Text` placeholders for `Loading` / `Empty` / `Error`, `LazyColumn` of `ConversationRow`s keyed by `Conversation.id` for `Loaded`); dispatches `ChannelListEvent.RowTapped` and `ChannelListEvent.SettingsTapped` via the destination-level `onEvent` lambda. Introduces the `koinViewModel<…>()` + `collectAsStateWithLifecycle()` call-site shape at the `channel_list` NavHost destination, and pulls `androidx.compose.material:material-icons-core` onto the classpath for `Icons.Default.Settings`.
