...
Observes the persistent-channel slice + the unpromoted-discussions list from the conversation repository, folds them together via `combine`, and exposes the result as a `StateFlow<ChannelListUiState>`. Emits `Loading` initially (before *either* upstream produces a value — `combine` gates first emit on both), then `Empty(recentDiscussions, recentDiscussionsCount)` or `Loaded(channels, recentDiscussions, recentDiscussionsCount)` depending on the channels list, and `Error(message)` if either upstream flow throws. Since #69 the Discussions upstream lands as both `recentDiscussions = discussions.take(3)` (the top-3 surface consumed by the inline section's preview rows) and `recentDiscussionsCount = discussions.size` (the total used by the "See all discussions (N) →" link). Both values come from a single `combine` emission — one upstream subscription.
...
class ChannelListViewModel(
    ...
    val state: StateFlow<ChannelListUiState> =
        combine(
            ...
            if (channels.isEmpty()) ChannelListUiState.Empty(recent, count)
            else ChannelListUiState.Loaded(channels, recent, count)
        }
            .catch { e ->
                val raw = e.message
...
```
channelsFlow ──┐                                             ┌─ recent = discussions.take(3)
               ├─ combine(channels, discussions) ──► derive ─┤
discussionsFlow ┘                                            └─ count  = discussions.size
                                                  ──► Empty(recent, count)       (channels.isEmpty())
                                                  └─► Loaded(ch, recent, count)  (otherwise)
                 ─── catch ─► Error(message)
...
`app/src/test/java/de/pyryco/mobile/ui/conversations/list/ChannelListViewModelTest.kt`. JUnit 4, matching the existing test-class style. Sixteen tests (six existing reshaped + two new in #69 to pin the `recentDiscussions` surface):
