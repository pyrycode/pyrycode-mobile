...
Boots the app into the route that matches the persisted pairing state (`welcome` on a fresh install, `channel_list` once `AppPreferences.pairedServerExists` is `true`) and provides the route graph that subsequent screens plug into. Currently seven routes:
...
- **`settings`** — renders `SettingsScreen` (#64; About-row wiring #90 / #91). Reached by the channel-list TopAppBar's gear `IconButton` (#21) via `ChannelListEvent.SettingsTapped → navController.navigate(Routes.Settings)`. Destination block passes two navigation lambdas: `onBack = { navController.popBackStack() }` and `onOpenLicense = { navController.navigate(Routes.License) }` (#91). See [Settings screen](settings-screen.md).
- **`license`** — renders `LicenseScreen` (#91), a generic M3 secondary screen displaying the bundled `assets/LICENSE` text. Reached from the Settings About-section License row. Destination block: `composable(Routes.License) { LicenseScreen(onBack = { navController.popBackStack() }) }`. See [License screen](license-screen.md).
