...
class DiscussionListViewModelTest {
    ...
    @Test
    fun loaded_containsOnlyUnpromoted_inSourceOrder() = runTest {
        val source = MutableSharedFlow<List<Conversation>>(replay = 0)
        val captured = mutableListOf<ConversationFilter>()
        val repo = object : ConversationRepository {
    ...
    @Test
    fun rowTapped_emitsToThreadNavigation() = runTest {
        val source = MutableSharedFlow<List<Conversation>>(replay = 0)
        val vm = DiscussionListViewModel(stubRepo(source))
    ...
    private fun sampleDiscussion(id: String): Conversation = Conversation(
        id = id,
        name = null,
